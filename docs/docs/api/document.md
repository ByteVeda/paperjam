...
Document metadata as a frozen `Metadata` dataclass. See [types reference](types).
...
See the [Async guide](../guides/async) for usage. Async versions use the `a` prefix: `asave`, `asave_bytes`, `arender_page`, `arender_pages`, `aextract_tables`, `ato_markdown`, `asearch`, `adiff`, `aredact_text`.
