...
pub fn validate_pdf_a(doc: &Document, level: PdfALevel) -> Result<ValidationReport> {
    ...
    // 1. Encryption check — PDF/A forbids encryption
    check_encryption(inner, &mut issues);

    // 2. XMP metadata check
...
/// Check if the document is encrypted (PDF/A forbids encryption).
...
fn check_xmp_metadata(doc: &lopdf::Document, level: PdfALevel, issues: &mut Vec<ValidationIssue>) {
    ...
    // Check for pdfaid:part and pdfaid:conformance
...
/// Check that all fonts have embedded font programs.
fn check_font_embedding(
    ...
    for (&page_num, &page_id) in page_map {
        ...
        // Get resources, handling both inline and referenced
        ...
        if let Some(font_dict) = fonts {
            for (_, font_ref) in font_dict.iter() {
                ...
                // Check for /FontDescriptor with /FontFile, /FontFile2, or /FontFile3
...
/// Check for required OutputIntents with ICC profile.
...
/// Check for transparency (forbidden in PDF/A-1).
fn check_transparency(
    ...
    for (&page_num, &page_id) in page_map {
        ...
        // Check ExtGState for /SMask and /BM
        let resources = get_resources(doc, page_dict);
...
/// Check for JavaScript actions (forbidden in PDF/A).
fn check_javascript(
    doc: &lopdf::Document,
    page_map: &std::collections::BTreeMap<u32, lopdf::ObjectId>,
    issues: &mut Vec<ValidationIssue>,
    ...
    // Check catalog-level /AA (additional actions)
    check_action_dict_for_js(doc, catalog, b"AA", None, issues);

    // Check catalog-level /OpenAction
    ...
    // Check catalog-level /Names -> /JavaScript
...
/// Check an additional-actions dictionary for JavaScript.
...
/// Check if an action object is a JavaScript action.
