    ...
    (data: Uint8Array, name: string) => {
      ...
      try {
        ...
        for (let i = 1; i <= count; i++) {
          try {
            pageInfos.push(d.pageInfo(i));
          } catch {
            pageInfos.push({ number: i, width: 0, height: 0, rotation: 0 });
          }
        }
        setPages(pageInfos);
    ...
    <div>
      ...
      {doc && (
        <>
          <Tabs
            tabs={TABS.map((t) =>
              t.id === 'pages' ? { ...t, label: `Pages (${pageCount})` } : t,
            )}
            active={activeTab}
          ...
          {activeTab === 'pages' && (
            <div>
              <p>
                <strong>{pageCount}</strong> page{pageCount !== 1 ? 's' : ''} in
                this document.
              </p>
              <div className={styles.pageDimensions}>
                {pages.map((p, i) => (
                  <div key={i} className={styles.pageCard}>
                    ...
                    <span>
                      {p.width.toFixed(1)} x {p.height.toFixed(1)} pt
                    </span>
                    {p.rotation !== 0 && (
                      <span> | Rotation: {p.rotation}&deg;</span>
                    )}
                  </div>
                ))}
              </div>
